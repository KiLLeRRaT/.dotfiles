...
function waLoadConfig() {
	...
	# change the below to also work if $CONFIG_PATH is a named pipe
...
function waCheckContainerRunning() {
	...
	echo "HERE WE ARE:"
	sudo "$WAFLAVOR" ps --all --filter name="winapps-1" --format '{{.Status}}'

    CONTAINER_STATE=$(sudo "$WAFLAVOR" ps --all --filter name="winapps-1" --format '{{.Status}}')
	echo "Container state command is: $CONTAINER_STATE"
    ...
    case "$WAFLAVOR" in
        "docker") COMPOSE_COMMAND="sudo docker compose" ;;
        "podman") COMPOSE_COMMAND="podman-compose" ;;
    esac
	echo "Compose command is: $COMPOSE_COMMAND"
